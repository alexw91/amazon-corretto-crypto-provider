...
SYMBOLS_FILE="libcrypto.original_symbols"
...
# Next, we use awk to retrieve only the .data and .text symbols, as well as weak symbols. Uppercase letters are global, lowercase are local symbols
nm --extern-only ${LIBCRYPTO_DIRECTORY}/${LIBCRYPTO_SHARED_OBJECT} | awk '/ [D|T|W|w] /{print $3}' | LC_ALL=C sort | uniq > ${LIBCRYPTO_DIRECTORY}/${RAW_SYMBOLS_FILE}
...
# Mangle libcrypto to use the new symbol names, and remove the old ones
objcopy --redefine-syms=${LIBCRYPTO_DIRECTORY}/${REDEFINITION_FILE} ${LIBCRYPTO_DIRECTORY}/${LIBCRYPTO_SHARED_OBJECT}
objcopy --strip-symbols=${LIBCRYPTO_DIRECTORY}/${SYMBOLS_FILE} ${LIBCRYPTO_DIRECTORY}/${LIBCRYPTO_SHARED_OBJECT}
#objcopy --strip-unneeded ${LIBCRYPTO_DIRECTORY}/${LIBCRYPTO_SHARED_OBJECT}

# Now, mangle ACCP's shared library objects to use the new symbols
